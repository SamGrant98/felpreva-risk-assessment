...
const takeScreenshot = () => {
  ...
  window.audio.Play('cameraShutter')
  window.XR8.CanvasScreenshot.takeScreenshot({
    onProcessFrame: ({ctx}) => {
      ...
      // Convert the initial canvas (without watermark) to a blob for smallPreview
      canvas.toBlob((initialBlob) => {
        const initialImageURL = URL.createObjectURL(initialBlob)
        document.getElementById('small-preview').style.backgroundImage = `url(${initialImageURL})`
      }, 'image/png')
    ...
    .then((data) => {
      ...
      latestPhotoBlob = blob  // Store it globally

      // Create an object URL from the final blob (with watermark)
      const imageURL = URL.createObjectURL(blob)
      clearState()

      // Dispatch the event with the final blob
...
const shareLatestPhoto = async () => {
  if (!latestPhotoBlob) {
    console.warn('No photo blob available to share.')
    return
  }

  const file = new File([latestPhotoBlob], 'ar-photo.jpg', { type: 'image/jpeg' })

  if (navigator.canShare && navigator.canShare({ files: [file] })) {
    try {
      await navigator.share({
        title: 'AR Photo',
        text: 'Check out this AR photo!',
        files: [file],
      });
      console.log('✅ Photo shared successfully')
    } catch (error) {
      console.warn('❌ Photo share failed or was cancelled:', error)
    }
  } else {
